...
class World:
	def __init__(self, win, data, groups):
		# level size (for progress bar, etc.)
		self.rows = len(data) if data else 0
		self.cols = len(data[0]) if data and len(data)>0 else 0
		self.level_width = self.cols * tile_size
		self.level_height = self.rows * tile_size
		self.tile_list  = []
		self.win = win
		self.groups = groups
		...
		# HUD diamond (single icon)
		hud_diamond = Diamond((WIDTH//tile_size - 3) * tile_size, tile_size // 2)
		self.groups[3].add(hud_diamond)
...
class Player:
	...
	def update(self, pressed_keys, game_over, level_won, game_won):
		...
		if not game_over and not game_won:
			# Controls: arrows OR WASD (space still jumps)
			jump_pressed = pressed_keys[K_UP] or pressed_keys[K_w] or pressed_keys[K_SPACE]
			left_pressed = pressed_keys[K_LEFT] or pressed_keys[K_a]
			right_pressed = pressed_keys[K_RIGHT] or pressed_keys[K_d]

			if jump_pressed and not self.jumped:
			...
			# add gravity (uses float velocity, but applies integer movement)
			self.vel_y += GRAVITY
			...
			# Enemies: stomp to kill (jump on top), otherwise you die.
			player_next_rect = self.rect.copy()
			player_next_rect.x += dx
			player_next_rect.y += dy
			for enemy in list(self.groups[4]):
				enemy_hitbox = getattr(enemy, 'hitbox', enemy.rect)
				if enemy_hitbox.colliderect(player_next_rect):
					# Falling + coming from above = stomp
					stomp_margin = 10
					if self.vel_y > 0 and (self.rect.bottom <= enemy_hitbox.top + stomp_margin):
						enemy.kill()
...
class Slime(pygame.sprite.Sprite):
	def __init__(self, x, y):
		...
		# rect is used for drawing (groups draw using sprite.rect)
		self.rect = self.image.get_rect(topleft=(x, y))
		# Smaller hitbox (especially for the blue blob)
		self.hitbox = self.rect.inflate(-18, -12)
